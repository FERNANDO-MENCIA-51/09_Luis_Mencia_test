...
public class PermissionServiceImpl implements PermissionService {
    ...
    @Override
    public Flux<Permission> getAllPermissions(String module, Boolean status) {
        // Si no hay filtros, devolver todos
        if (module == null && status == null) {
            return permissionRepository.findAll();
        }

        // Si solo hay filtro de módulo
        if (module != null && status == null) {
        ...
        return permissionRepository.findByModuleAndStatus(module, status);
    }
    ...
    @Override
    public Mono<Permission> restorePermission(UUID id) {
        return permissionRepository.findById(id)
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("Permission not found with id: " + id)))
                .flatMap(permission -> {
                    permission.setStatus(true);

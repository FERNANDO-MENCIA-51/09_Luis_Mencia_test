...
public class PersonServiceImpl implements PersonService {
    ...
    private final edu.pe.vallegrande.AuthenticationService.repository.DocumentTypeRepository documentTypeRepository;
    ...
    public Mono<PersonResponseDto> createPerson(PersonRequestDto personRequestDto) {
        ...
        return documentTypeRepository.findById(personRequestDto.getDocumentTypeId())
                .switchIfEmpty(Mono.error(new ResourceNotFoundException(
                        "Tipo de documento no encontrado con ID: " + personRequestDto.getDocumentTypeId())))
                .flatMap(documentType -> {
                    // Validar longitud del documento
                    int documentLength = personRequestDto.getDocumentNumber().length();
                    if (documentLength != documentType.getLength()) {
                        return Mono.error(new IllegalArgumentException(
                                String.format(
                                        "El número de documento debe tener exactamente %d caracteres para el tipo %s. Se recibieron %d caracteres",
                                        documentType.getLength(), documentType.getDescription(), documentLength)));
                    }

                    // Validar que el tipo de documento esté activo
                    if (!documentType.getActive()) {
                        return Mono.error(new IllegalArgumentException(
                                "El tipo de documento " + documentType.getDescription() + " no está activo"));
                    }

                    return Mono.just(documentType);
                })
                .then(personRepository.existsByDocumentTypeIdAndDocumentNumber(
                            ...
                            .flatMap(documentType -> {
                                ...
                                if (documentLength != documentType.getLength()) {
                                    return Mono.error(new IllegalArgumentException(
                                            String.format(
                                                    "El número de documento debe tener exactamente %d caracteres para el tipo %s. Se recibieron %d caracteres",
                                                    documentType.getLength(), documentType.getDescription(),
                                                    documentLength)));
                                }
                                ...
                                if (!documentType.getActive()) {
                                    return Mono.error(new IllegalArgumentException(
                                            "El tipo de documento " + documentType.getDescription()
                                                    + " no está activo"));
                                }
